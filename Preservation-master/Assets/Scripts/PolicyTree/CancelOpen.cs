...
public class CancelOpen : MonoBehaviour
{
    ...
    {
        if (CancelTransitDescription != null)
        {

            //booleans for whether each policy is description is open or not
            bool isActive = CancelTransitDescription.activeSelf;
            ...
            CancelTransitDescription.SetActive(!isActive);

            //if the other policies are open
            if (indoorActive == true || over100Active == true || publicActive == true ||
                nonEssentialTravelActive == true || reduceTransitActive == true || parksActive == true ||
