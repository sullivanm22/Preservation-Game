...
public class NonEssentialOpen : MonoBehaviour
{
    ...
    {
        if (NonEssentialTravelDescription != null)
        {

            //booleans for whether each policy is description is open or not
            bool isActive = NonEssentialTravelDescription.activeSelf;
            ...
            NonEssentialTravelDescription.SetActive(!isActive);

            //if the other policies are open
            if (indoorActive == true || over100Active == true || publicActive == true ||
                parksActive == true || reduceTransitActive == true || cancelTransitActive == true ||
